...
xlength = length(idxs_ymin); % number of cells in x direction
ymax = max(vertices(2,:));
...
xmin = min(vertices(1,:));
idxs_xmin = find(vertices(1,:) == xmin);
ylength = length(idxs_xmin); % number of cells in y direction
xmax = max(vertices(1,:));
idxs_xmax = find(vertices(1,:) == xmax);
...
% una_movavg = una; cna_movavg = cna;
% for iii = 1:ylength
%     for jjj=idxs_xmin(iii) + semi_amp_movavg + 1 : xlength + idxs_xmin(iii) - semi_amp_movavg-1
%         una_movavg(jjj) = nanmean(una(jjj-semi_amp_movavg:jjj+semi_amp_movavg));
%         cna_movavg(jjj) = nanmean(cna(jjj-semi_amp_movavg:jjj+semi_amp_movavg));
%     end
% end
% una = una_movavg; cna = cna_movavg;
        maxuwet = max(abs(u_ex(wetnodes)));
        una(drynodes) = max(una(drynodes),-maxuwet);
        una(drynodes) = min(una(drynodes),maxuwet);
...
% un_movavg = un; cn_movavg = cn;
% for iii = 1:ylength
%     for jjj=idxs_xmin(iii) + semi_amp_movavg + 1 : xlength + idxs_xmin(iii) - semi_amp_movavg-1
%         un_movavg(jjj) = nanmean(un(jjj-semi_amp_movavg:jjj+semi_amp_movavg));
%         cn_movavg(jjj) = nanmean(cn(jjj-semi_amp_movavg:jjj+semi_amp_movavg));
%     end
% end
% un = un_movavg; cn = cn_movavg;
    maxuwet = max(abs(u_ex(wetnodes)));
    un(drynodes) = max(un(drynodes),-maxuwet);
    un(drynodes) = min(un(drynodes),maxuwet);
    disp('max(u) max(v) max(c)')
