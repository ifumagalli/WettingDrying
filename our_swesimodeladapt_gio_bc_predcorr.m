...
wetnodes = find_wetnodes(boundaries,cn,g,wdtol,'pred');
wetdof = [wetnodes,wetnodes+nov,wetnodes+2*nov]';
drydof = setdiff(dof,wetdof);
drynodes = drydof(1:length(wetnodes));
...
% Linear system
    if ~isempty([uDir_in,vDir_in,cDir]) %eliminazione dei nodi di Dirichlet
        rhs = rhs(dof,1) - aglo(dof,Dnodes)*[uDir_in,vDir_in,cDir,cDir_in]';
        temp = aglo(dof,dof)\rhs;
...
%         cn(nodes0,1) = cDir_in;
%         cn(nodes1,1) = cDir(length(nodes2)+1:length(DirDof_for_c));
    else
        temp = aglo\rhs;
        un(dof_uv_in,1) = temp(1:ndof_uv_in,1);
        vn(dof_uv_in,1) = temp(ndof_uv_in+1:2*ndof_uv_in,1);
