function [u_ex,h_ex,shockvelocity_ex,wd_ex] = exact_Stoker_Ritter(vertices,elements,boundaries,save_path,tspan,h0,hl,hr,g,n,do_print,slope)
...
if(slope==0)
    ...
    delta = 0; % angle of friction
    m = g*(cos(slope)*tan(delta)-sin(slope));
    c0 = sqrt(g*wdl*cos(slope));
    ex_xl = @(t) 2*c0*t-0.5*m*t.^2;
    ex_xr = @(t) c0*t+0.5*m*t.^2;
    
    ex_wdco = @(x,t) 1/(9*g*cos(slope))*(2*c0-x*(1./t)-(m*ones(size(x))*t)/2).^2;
    
    ex_wd = @(x,t) wdl*(x*ones(size(t))<=-ex_xr(ones(size(x))*t)) ...
                + ex_wdco(x,t).*(x*ones(size(t))<ex_xl(ones(size(x))*t)).*(x*ones(size(t))>-ex_xr(ones(size(x))*t)) ...
                + wdr*(x*ones(size(t))>=ex_xl(ones(size(x))*t));
    wd_ex = ex_wd(vertices(1,:)',t);
    
    h_ex = wd_ex + h0*ones(size(t));
